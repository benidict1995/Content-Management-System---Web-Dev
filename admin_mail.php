...
<div id="filteruser">
	<form action="admin_mail.php" method="post" enctype="multipart/form-data">
...
<?php
	if (isset($_POST['filter']))
	{
		...
		$result = mysql_query("SELECT * FROM table_shipping WHERE buyer_username LIKE '%" .$username. "%'");
		echo "<table class='table table-bordered table-hover' style='width:100%; text-align:center;'>

		<tr>
		<td id='menutitle'>Username</td>
		<td id='menutitle'>Shipping</td>
		<td id='menutitle'>Name</td>
		<td id='menutitle'>Address</td>
		<td id='menutitle'>Contact Number</td>
		<td id='menutitle'>Item Size</td>
		<td id='menutitle'>Product</td>
		...
		while ($row = mysql_fetch_array($result)) 
		{
			...
			echo "<td>".$row['padala_name']."</td>";
			echo "<td>".$row['complete_name']."</td>";
			echo "<td>".$row['complete_address']."</td>";
			echo "<td>".$row['contact_number']."</td>";
			echo "<td>".$row['item_size']."</td>";
			echo "<td>"."<img src='shipping_item/{$row['name_upload']}' style='width:300px; height:150px;'>"."</td>";
			echo "<td>".'<a href="msgdelete.php?shipping_id='.$row['shipping_id'].'">Delete</a>'. "/" .'<a href="#adminreply?costumer='.$row['buyer_username'].'">Reply</a>'."</td";
			echo "</tr>";
		}
		echo "</table>";
	...
	{
		$result = mysql_query("SELECT * FROM table_shipping");
		echo "<table class='table table-bordered table-hover' style='width:100%; text-align:center;'>
		...
		while ($row = mysql_fetch_array($result)) 
		{
			...
			echo "<td>".$row['item_size']."</td>";
			echo "<td>"."<img src='shipping_item/{$row['name_upload']}' style='width:300px; height:150px;'>"."</td>";
			echo "<td>".'<a href="msgdelete.php?shipping_id='.$row['shipping_id'].'">Delete</a>'. "/" .'<a href="reply.php?customer='.$row['buyer_username'].'"  >Reply</a>'."</td>";
